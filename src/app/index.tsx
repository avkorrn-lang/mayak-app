...
const SYMPTOM_LINKS: Record<string, string[]> = {
  ...
  'Жар': ['ТРУД', 'STOP'],
  'Дрожь': ['STOP', 'ТРУД', 'Серфинг эмоций'],
  ...
  'Потливость': ['ТРУД', 'STOP'],
  'Ком в горле': ['Проверка фактов', 'Радикальное принятие'],
};
...
const TECHNIQUE_ID_MAP: Record<string, string> = {
  ...
  'STOP': 'stop',
  'ТРУД': 'trud',
  'Накопление положительных эмоций': 'positive',
};
...
function isPurePositive(emotions: string[]): boolean {
  return emotions.length > 0 && emotions.every(e => e === 'Радость' || e === 'Любовь');
}
...
export default function CompassScreen() {
  ...
  const safeIntensity = intensity ?? 0; const recommended = intensity !== null && bodyMode !== null && !(positiveOnly && safeIntensity < 65)
    ? getRecommendedTechniques(bodyMode, chosenSymptoms, chosenEmotions, chosenUrges, safeIntensity)
    : [];
  ...
  const handleBodyContinue = () => {
    ...
    const level = getLevelFromScores(symptomCount, urgeCount);
    setIntensity(level);
    setStep('result');
  };
        ...
        <SafeAreaView style={styles.safe}>
          <ScrollView contentContainerStyle={styles.scroll}>
            <View style={styles.iconContainer}>
              <MaterialCommunityIcons name="lighthouse" size={80} color={colors.accent} />
            </View>
            ...
            <Text style={[Fonts.subtitle, { color: colors.textSecondary, textAlign: 'center', marginBottom: Spacing.md }]}>
              Твой навигатор по техникам самопомощи
            </Text>
            <View style={styles.card}>
              <Text style={styles.welcomeTitle}>Что здесь происходит?</Text>
              <Text style={styles.welcomeText}>
                «Маяк» — это безопасный инструмент, который поможет тебе понять, что ты чувствуешь прямо сейчас, и подобрать подходящую технику самопомощи.
              </Text>
              <Text style={styles.welcomeTitle}>Два пути:</Text>
              <View style={styles.row}>
                <TouchableOpacity
                  style={[styles.methodBtn, { backgroundColor: colors.surface, borderColor: colors.border }]}
                  onPress={() => { setBodyMode(true); setStep('body'); }}
                  activeOpacity={0.7}
                >
                  ...
                  <Text style={[styles.methodDesc, { color: colors.textSecondary }]}>Оценить ощущения в теле и позывы → получить техники</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.methodBtn, { backgroundColor: colors.surface, borderColor: colors.border }]}
                  onPress={() => { setBodyMode(false); setStep('emotions'); }}
                  activeOpacity={0.7}
                >
                  <MaterialCommunityIcons name="speedometer" size={32} color={colors.accent} />
                  <Text style={[styles.methodTitle, { color: colors.text }]}>Шкала</Text>
                  <Text style={[styles.methodDesc, { color: colors.textSecondary }]}>Отметить эмоции и выбрать интенсивность → получить техники</Text>
                </TouchableOpacity>
              </View>
              <Text style={styles.welcomeText}>
                После оценки вы получите список рекомендуемых техник, которые можно сразу открыть.
              </Text>
            </View>
          </ScrollView>
        </SafeAreaView>
  ...
  return (
    <Background>
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.scroll}>
          <View style={styles.iconContainer}>
            <MaterialCommunityIcons name="lighthouse" size={64} color={colors.accent} />
          </View>
          <Text style={[Fonts.title, { color: colors.text, textAlign: 'center' }]}>Маяк</Text>
          <Text style={[Fonts.subtitle, { color: colors.textSecondary, textAlign: 'center', marginBottom: Spacing.md }]}>
          ...
          {step === 'body' && (
            <View style={styles.card}>
              ...
              {SYMPTOMS.map((s, i) => {
                ...
                return (
                  <TouchableOpacity key={i} style={[styles.checkRow, selected && styles.checkRowActive]} onPress={() => toggleSymptom(i)} activeOpacity={0.7}>
                    <MaterialCommunityIcons name={selected ? 'checkbox-marked' : 'checkbox-blank-outline'} size={20} color={selected ? colors.accent : colors.textSecondary} />
              ...
              <View style={styles.buttonGroup}>
                <StyledButton title="Получить техники" onPress={handleBodyContinue} disabled={!canBodyContinue} />
                <TouchableOpacity onPress={() => reset()} style={styles.skipBtn}>
                  <Text style={[styles.skipText, { color: colors.textSecondary }]}>Вернуться к выбору</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
          ...
          {step === 'emotions' && (
            <View style={styles.card}>
              ...
              <View style={styles.buttonGroup}>
                <StyledButton title="Продолжить →" onPress={() => setStep('intensity')} disabled={!canEmotionContinue} />
                <TouchableOpacity onPress={() => reset()} style={styles.skipBtn}>
                  <Text style={[styles.skipText, { color: colors.textSecondary }]}>Вернуться к выбору</Text>
                </TouchableOpacity>
              </View>
          ...
          {step === 'result' && levelInfo && (
            <View style={[styles.card, { borderLeftColor: levelInfo.color, borderLeftWidth: 4 }]}>
              ...
              {positiveOnly && safeIntensity < 65 ? (
                <View style={{ marginVertical: 12 }}>
                  <Text style={[styles.positiveMessage, { color: colors.accent }]}>
                    Похоже, вы чувствуете радость или любовь — это прекрасно! Сейчас вам не нужны техники, просто наслаждайтесь. Если захотите закрепить это состояние, попробуйте «Накопление положительных эмоций».
                  </Text>
                  <TouchableOpacity
                    style={styles.techBtn}
                    onPress={() => router.push({ pathname: '/techniques', params: { open: 'positive' } })}
                    activeOpacity={0.7}
                  >
                    <Text style={[styles.techBtnText, { color: colors.text }]}>Накопление положительных эмоций</Text>
